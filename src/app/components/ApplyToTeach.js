...
export default function ApplyToTeach({ isOpenApplytoTeach, setIsOpenApplytoTeach }) {
    ...
    const handleNext = () => {
        if (currentStep === 1 && !selectedOption) return alert('Please select an option.');
        if (currentStep === 2 && !language) return alert('Please select a language.');
        if (currentStep === 3 && !phone) return alert('Please enter your phone number.');
        if (currentStep === 4 && !email) return alert('Please enter your email.');
                                ...
                                {!isStepMode ? (
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', marginTop: '-35px', padding: '0100px', height: '100%' }}>
                                        ...
                                        <div style={{ marginTop: '20px', display: 'flex', justifyContent: 'end', gap: '10px' }}>
                                            ...
                                            {currentStep === 5 && (
                                                <button className="btn pl-3 pr-2 d-flex align-items-center" style={{ background: '#13C5CD', color: '#fff', fontSize: '14px', borderRadius: '1px' }} onClick={handleSubmit}>
